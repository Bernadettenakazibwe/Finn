...
def simple_calibration(cap, detector, predictor, samples=30):
    """
    Captures `samples` frames of the user looking straight at the camera,
    then returns average left/right pupil radii as baselines.
    """
    ...
    while count < samples:
        ...
        le = crop_eye(frame, left_region)
        re = crop_eye(frame, right_region)

        Lr = detect_pupil_size(le)
        Rr = detect_pupil_size(re)
        if 1 < Lr < 80 and 1 < Rr < 80:
            sum_L += Lr
            sum_R += Rr
            count += 1

        cv2.putText(frame, f"Calibrating: {count}/{samples}", (10,30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,0), 2)
        cv2.imshow("Calibration", frame)
        cv2.waitKey(1)

    cv2.destroyWindow("Calibration")
    baseline_L = sum_L / samples
    baseline_R = sum_R / samples
...
if not calibration_data:
    baseline_left, baseline_right = simple_calibration(cap, detector, predictor)
    calibration_data = {
        "straight": {
            "left": baseline_left,
